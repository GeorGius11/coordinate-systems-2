...
const initialWindowSizes = Array.from({ length: 100 }, (_, index) => index + 1);
let optimalWindowSize = initialWindowSizes[0];
let bestPerformance = Number.POSITIVE_INFINITY;

initialWindowSizes.forEach((windowSize) => {
  const filteredNoise = movingAverageFilter(noise, windowSize);
  const performanceMetric = standardDeviation(filteredNoise);
...
console.log(
  `Optimal Window Size: ${optimalWindowSize}, Best Performance: ${bestPerformance}`
);

const filteredSignal = movingAverageFilter(signal, optimalWindowSize);
const filteredNoise = movingAverageFilter(noise, 5);
const filteredSignalPlusNoise = movingAverageFilter(
  signalPlusNoise,
  optimalWindowSize
);
